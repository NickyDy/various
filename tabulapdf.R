...
link <- "https://www.dksbt.bg/doc/%D0%A1%D0%B5%D0%B4%D0%BC%D0%B8%D1%87%D0%B5%D0%BD%20%D0%B1%D1%8E%D0%BB%D0%B5%D1%82%D0%B8%D0%BD%20%2017%20-%2021%20%D0%BD%D0%BE%D0%B5%D0%BC%D0%B2%D1%80%D0%B8%20NEW%202025.pdf"
...
table <- extract_tables(link, col_names = F, method = "stream", pages = 1, output = "tibble") %>% pluck(1) %>% 
  ...
  separate_wider_delim(cols = X8, names = c("X8", "X8_1", "X9", "X9_1", "X10", "X10_1", 
                                            "X11", "X11_1", "X12"), delim = " ") %>% 
  select(product = X1, unit = X2, "2025-11-17" = X4, "2025-11-18" = X6, "2025-11-19" = X8, 
         "2025-11-20" = X9_1, "2025-11-21" = X11) %>% 
